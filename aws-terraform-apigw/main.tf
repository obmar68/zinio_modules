...
#              convention.
module "labels" {
  source  = "git@github.com:obmar68/zinio_modules.git//aws-terraform-labels"
  //version = "0.15.0"
...
resource "aws_api_gateway_method" "default" {
  count = length(var.path_parts) > 0 ? length(var.path_parts) : 1

  rest_api_id          = aws_api_gateway_rest_api.default.*.id[0]
  resource_id          = length(var.path_parts) > 0 ? aws_api_gateway_resource.default.*.id[count.index] : aws_api_gateway_rest_api.default.*.root_resource_id[0]
...
resource "aws_api_gateway_integration" "default" {
  ...
  cache_namespace         = length(var.cache_namespaces) > 0 ? element(var.cache_namespaces, count.index) : aws_api_gateway_rest_api.default.*.root_resource_id[0]
...
resource "aws_api_gateway_method_response" "head_200" {
  ...
  rest_api_id = aws_api_gateway_rest_api.default.*.id[0]
  resource_id = length(var.path_parts) > 0 ? aws_api_gateway_resource.default.*.id[count.index] : aws_api_gateway_rest_api.default.*.root_resource_id[0]
  http_method = aws_api_gateway_method.head_method.*.http_method[count.index]
  status_code = "200"
  ...
  depends_on = [aws_api_gateway_method.head_method]
}
...
resource "aws_api_gateway_integration_response" "head_integration_response" {
  count       = length(aws_api_gateway_integration.head_integration.*.id)
  rest_api_id = aws_api_gateway_rest_api.default.*.id[0]
  resource_id = length(var.path_parts) > 0 ? aws_api_gateway_resource.default.*.id[count.index] : aws_api_gateway_rest_api.default.*.root_resource_id[0]
  http_method = aws_api_gateway_method.head_method.*.http_method[count.index]
  status_code = aws_api_gateway_method_response.head_200.*.status_code[count.index]
  ...
  depends_on = [
    aws_api_gateway_method_response.head_200,
...
resource "aws_api_gateway_deployment" "default" {
  ...
  triggers = {
    redeployment = sha1(jsonencode(aws_api_gateway_rest_api.default.*.body[0]))
  }
