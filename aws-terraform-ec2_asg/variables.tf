...
variable "instance_profile_override" {
  description = "Optionally provide an instance profile. Any override profile should contain the permissions required for provider support tooling to continue to function if required."
  type        = bool
  default     = false
}

variable "instance_profile_override_name" {
  description = "Provide an instance profile name. Any override profile should contain the permissions required for provider support tooling to continue to function if required. To use this set `instance_profile_override` to `true`."
  type        = string
  default     = ""
}
...
variable "provider_alarms_enabled" {
  description = "Specifies whether alarms will create a Provider ticket.  Ignored if provider_managed is set to false."
  type        = bool
  default     = false
}

variable "provider_managed" {
  description = "Boolean parameter controlling if instance will be fully managed by provider support teams, created CloudWatch alarms that generate tickets, and utilize provider managed SSM documents."
  type        = bool
  default     = true
}
